...
class WaitingMeetingListView(APIView, MyPaginationMixin):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if (request.user.id == 5 or request.user.id == 120 or request.user.id == 100 or request.user.id == 85):
                pass
            else:
                filtered_data = filtered_data.filter(~Q(openby__gender=request.user.gender))
